...
export class Joint extends Drawable_Scene_Object {
    // ############################################################################################
    // ############################################################################################
    // ############################################################################################
    ...
    generate_mesh_data(){
        ...
        // --------------------------------------------------------
        this.joint_shape = Joint.prepare_shape();
    ...
    static prepare_shape(){
        ...
        // three loops for each of the planes

        // first is the XY plane
        for (let i = 0; i < circle_points.length; i++) {
            const point = circle_points[i];
            
            shape_points.push({x: point.x, y: point.y, z: 0.0});
        }
        // next is the YZ plane
        for (let i = 0; i < circle_points.length; i++) {
            const point = circle_points[i];
            
            shape_points.push({x: 0.0, y: point.x, z: point.y});
        }
        // next is the ZX plane
        for (let i = 0; i < circle_points.length; i++) {
            const point = circle_points[i];
            
            shape_points.push({x: point.y, y: 0.0, z: point.x});
        }
        ...
        // first is the XY plane
        for (let i = 0; i < circle_points.length; i++) {
            const current = shape_points[i];
            // wrap the index around to the first of this circle's points
            const next = shape_points[(i+1)%circle_points.length];
            
            shape_factory.add_edge_with_colour_sizes(current,next,XY_colour,ring_point_size,ring_point_size);
        }
        ...
        // next is the YZ plane
        for (let i = 0; i < circle_points.length; i++) {
            // skip first circle's points
            const current = shape_points[circle_points.length + i];
            // wrap the index around to the first of this circle's points
            const next = shape_points[circle_points.length + (i+1)%circle_points.length];
            
            shape_factory.add_edge_with_colour_sizes(current,next,YZ_colour,ring_point_size,ring_point_size);
        }
        ...
        // next is the ZX plane
        for (let i = 0; i < circle_points.length; i++) {
            // skip first two circle's points
            const current = shape_points[(2*circle_points.length) + i];
            // wrap the index around to the first of this circle's points
            const next = shape_points[(2*circle_points.length) + (i+1)%circle_points.length];
            
            shape_factory.add_edge_with_colour_sizes(current,next,ZX_colour,ring_point_size,ring_point_size);
        }
